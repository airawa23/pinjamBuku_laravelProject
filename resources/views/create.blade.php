...
<body class="text-white font-sans antialiased">
    <div class="max-w-2xl mx-auto px-6 py-16">
        ...
        <div class="bg-[#14213d] rounded-2xl p-8 border border-slate-800 shadow-2xl">
            <h1 class="text-2xl font-bold mb-6 text-blue-400"><i class="fas fa-book-medical mr-2"></i> Tambah Buku Baru</h1>
            ...
            <form action="{{ route('books.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="title" required class="w-full bg-black/20 border border-slate-800 rounded-xl px-4 py-3 mt-1.5 text-sm focus:outline-none focus:border-blue-500 transition">
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-xs font-semibold text-gray-400 uppercase">Penulis / Author</label>
                        <input type="text" name="author" required class="w-full bg-black/20 border border-slate-800 rounded-xl px-4 py-3 mt-1.5 text-sm focus:outline-none focus:border-blue-500 transition">
                    </div>
                    <div>
                        ...
                        <select name="category" required class="w-full bg-[#14213d] border border-slate-800 rounded-xl px-4 py-3 mt-1.5 text-sm focus:outline-none focus:border-blue-500 transition">
                            ...
                            <option value="Environment & Life">Environment & Life</option>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="number" name="price" required class="w-full bg-black/20 border border-slate-800 rounded-xl px-4 py-3 mt-1.5 text-sm focus:outline-none focus:border-blue-500 transition">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-400 uppercase">Upload Cover Buku</label>
                        <input type="file" name="image" required accept="image/*" class="w-full bg-black/20 border border-slate-800 rounded-xl px-4 py-2.5 mt-1.5 text-sm focus:outline-none focus:border-blue-500 transition file:mr-4 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-blue-500/20 file:text-blue-400 hover:file:bg-blue-500/30">
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-gray-400 uppercase">Deskripsi Buku</label>
                    <textarea name="description" rows="4" required class="w-full bg-black/20 border border-slate-800 rounded-xl px-4 py-3 mt-1.5 text-sm focus:outline-none focus:border-blue-500 transition"></textarea>
                </div>
